...
- 새로운 루틴이 호출된 후 완전히 종료되기 전, 해당 루틴에서 사용했던 정보들을 보관하고 있어야 함 (중단, 재개라는 개념이 존재하기 때문) 즉, 루틴이 중단되었다가 다시 해당 메모리에 접근이 가능함
...
```
START, Thread: main @coroutine#1
LAUNCH END, Thread: main @coroutine#2
END Thread: main
```
</details>
...
- 코루틴의 의미
  - 경량 쓰레드 (왜 경량 쓰레드 인지)
  - CoroutineDispatcher 작업 대기열
  - 예외 전파
  - Job
- Dispatcher IO, Dispatcher Default 차이
- join, await