...
대신 EC2 생성(jar 배포) 등등 세세한 부분까지 다루지는 않고 큰 부분들만 다루어보겠습니다. (그래서 이 글은 따라하면서 보기는 애매할 수 있어 실습을 해보고 싶다면 제 블로그 다른 [AWS 관련 글](https://devlog-wjdrbs96.tistory.com/category/Cloud/AWS) 들을 많이 참고해주세요. 이 글은 그냥 이렇게 했구나 하고 보기만 하는!)
...
이제 이 글의 핵심인 `CodeDeploy`, `Load-Balancer` 설정은 어떻게 했고, `Docker` 배포 Shell Script는 어떻게 작성했는지에 대해서 알아보겠습니다. 
...
Github에 push를 할 때마다 자동으로 Jenkins 빌드가 수행되고, 문제가 없다면 위와 같이 잘 성공하는 것을 볼 수 있습니다. 
...
그리고 Jenkins가 빌드를 마친 후에 S3로 파일을 업로드 하고 CodeDeploy 배포가 진행될 때 단계들은 위와 같습니다.(어떤 여러 배포 단계들을 거쳐서 수행하는데 이것에 대한 설명은 AWS 공식문서를 보시면 나와 있습니다.)  
...
그리고 EC2 인스턴스가 2개 이다 보니 위처럼 2개가 보이는데 인스턴스 하나 배포하는데 대략 3분 정도가 걸렸습니다.  
...
지금까지 CodeDeploy로 배포를 하고 있는 것들인데요. 현재는 큰 문제 없이 배포가 안정화가 되어 `무중단 자동화 배포`가 계속 진행되고 있습니다. 

<br>

## `글을 마무리 하며`

매번 자동화 배포 해야지 했는데 이번 기회에 적용할 수 있게 되었습니다. 기존에는 jar를 직접 빌드하고 FileZila를 통해서 EC2에 업로드 했는, 이제는 Github에 push만 하면 모든게 자동으로 진행되니 너무 편리한 거 같습니다.
또한 배포할 때마다 서버가 중단되는 것이 아니라 `무중단`으로 진행하기 때문에 배포할 때마다 서비스가 중단되어야 하는 문제도 사라졌습니다. (아쥬 편하쥬?)

CI/CD는 서버 개발자라면 꼭 한번쯤은 해볼만 한 주제여서 기회가 된다면 진행하고 있는 프로젝트에 적용해보면 좋을 거 같습니다.