...
Cursor 라는 것은 `PK`처럼 고유한 값이 되면 단순하지만, 중복될 수 있는 값이라면 문제가 될 수 있다.

예를들어, 만약 Cursor 값이 name 이라면 중복된 값이 존재할 수 있다.

| id  | name  | salary | 노출 여부 |
|-----|-------|--------|-------|
| 1   | 철수    | 200    | O     |
| 2   | 철수  | 300    | X     |
| 3   | 철수 | 500    | X     | 
...
철수라는 이름이 중복된 상황에서 위의 쿼리를 실행하면 위의 4개 row를 모두 건너뛰게 됩니다. 즉, 현재 1번 철수만 클라이언트에 반환해줬던 것인데 2, 3, 4번 철수들도 건너뛰게 되는 문제점이 생깁니다.

이런 경우라면 ORDER BY에 id or timestamp 같이 중복되지 않은 값을 ORDER BY 정렬 조건에 추가해주어야 한다.
