...
`URL을 보면 Protocol, Host, Port 번호를 모두 합친 것이 출처(Origin)` 입니다. 두 개의 출처가 같다는 것은 Scheme, Host, Port 이 3가지가 동일하다는 뜻입니다. 프론트엔드와 백엔드의 출처가 다를 때, 다른 출처로 자원을 요청하면 CORS 에러가 납니다.
