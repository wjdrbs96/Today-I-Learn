...
컬렉션은 map 함수를 각 원소에 대해 먼저 수행해서 새 시퀀스를 얻고, 그 시퀀스에 대해 다시 filter를 수행할 것입니다. 하지만 시퀀스에 대한 map과 filter는 각 원소에 대해 순차적으로 적용됩니다. 즉, 첫 번째 원소가 처리되고, 다시 두 번째 원소가 처리 됩니다.
