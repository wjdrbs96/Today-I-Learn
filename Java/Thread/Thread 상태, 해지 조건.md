...
yield()는 쓰레드 자신에게 주어진 실행시간을 다음 차례의 쓰레드에게 `양보(yield)` 합니다. 예를들어, 스케쥴러에 의해 1초의 실행시간을 할당받은 쓰레드가 0.5초의 시간동안 작업한 상태에서 `yield()가 호출되면 나머지 0.5초는 포기하고 다시 실행대기 상태`가 됩니다.
